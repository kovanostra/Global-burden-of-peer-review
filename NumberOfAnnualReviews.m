...
reviews = inHouseAcceptanceRate.*2.3.*(round(reviewsAndRevisions(1)) + secondRoundPapers);

for iRevisions = 2:length(reviewsAndRevisions)
    secondRoundPapers = secondRoundProbability.*reviewsAndRevisions(iRevisions);
    reviews = reviews + inHouseAcceptanceRate.*2.3.*(iRevisions*reviewsAndRevisions(iRevisions) + secondRoundPapers);
end
